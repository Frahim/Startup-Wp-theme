...
Block::make(__('Latest Blog'))
        ->add_fields(array(
            Field::make('text', 'ltb_heading', __('Heading')),
            Field::make('textarea', 'ltb_content', __('Content')),
            Field::make('association', 'ltb_association', __('Association'))
            ->set_types(array(
                array(
                    'type' => 'post',
                    'post_type' => 'post',
            ...
            <section class="latest-blog-area section-padding">
                ...
                <div class="section-heading text-center mb-64">
                    <h2 class="dark-1 fw-bold wow animated fadeInUp">
                        <?php echo $fields['ltb_heading']; ?>  
                    </h2>
                    <?php
                    $content_blg_sec = $fields['ltb_content'];
                    if (!empty($content_blg_sec)) {
                        ?> 
                        <p class="dark-2 wow animated fadeInUp">
                            <?php echo $fields['ltb_content']; ?>  
                        </p>
                    <?php } ?>
                </div>
                <!-- section heading -->
                    ...
                    <div class="splide" role="group" id="latest_blog_post_slider" aria-label="Splide Basic HTML Example">
                        <div class="splide__track">
                            <ul class="splide__list">
                                <?php 
                                $i=0;
                                foreach ($fields['ltb_association'] as $blgitem) { ?>
                                    <?php
                                    $a = get_the_category($blgitem['id']);
                                    $featured_img_url = get_the_post_thumbnail_url($blgitem['id'], 'full');
                                    ?>
                                    <li class="splide__slide">
                                        <div class="single-latest-blog wow animated fadeInLeft" data-wow-delay="0.<?php echo $i;?>0s">
                                            ...
                                            <div class="single-latest-blog-content radius-8">
                                                ...
                                                <div class="date-category">
                                                    ...
                                                    <span class="post-date fs-14 fw-bold dark-2">
                                                        ...
                                                        <?php echo get_the_date('j F Y', $blgitem['id']) ?>

                                                    </span>
                                                </div>
                                                <!-- Date & Category -->
                                                <h4 class="post-title dark-1 fw-bold">
                                                    <?php echo get_the_title($blgitem['id']); ?>


                                                </h4>
                                                <p class="post-excerpt fs-20 dark-2">
                                                    <?php echo get_post_field('post_content', $blgitem['id']); ?>
                                                </p>
                                            </div>
                                            <!-- Read More btn -->
                                            <a href="<?php echo get_permalink($blgitem['id']) ?>"
